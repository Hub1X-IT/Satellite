...
public class RotationReset : MonoBehaviour
{
    ...
    [SerializeField]
    private WhenToReset whenToReset;

    [SerializeField]
    private RotationType rotationTypeToReset;
    ...
    {
        if (rotationTypeToReset == RotationType.GlobalRotation)
        {
            gameObjectToReset.transform.rotation = Quaternion.Euler(targetRotation);
        }
        else if (rotationTypeToReset == RotationType.LocalRotation)
        {
            gameObjectToReset.transform.localRotation = Quaternion.Euler(targetRotation);
        }
    }
}